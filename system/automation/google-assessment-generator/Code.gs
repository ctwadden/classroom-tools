...
function onOpen(){
  SpreadsheetApp.getUi().createMenu("Truck Standard")
    ...
    .addItem("2. Sync assessment specs","syncAssessmentSpecs")
...
function setupTechnologyEvidenceSystem(){
  ...
  if(c.getLastRow()<2)c.appendRow(["ASSESSMENT_SPECS_FOLDER_ID","","Drive folder containing forms-spec.json files"]);
}
...
function generateFormsForPendingSpecs(){
  ...
  rows.forEach(function(r,i){
    ...
    try{
      var spec=JSON.parse(DriveApp.getFileById(r[3]).getBlob().getDataAsString("UTF-8"));
